...
function findEslintEnvInfos(text)
{
    ...
    for (let match; match = eslintEnvPattern.exec(text);)
    {
        ...
        const eslintEnvInfo = { env, lineBreak, range: [start, end] };
...
function createReplacement(globals, lineBreak)
{
    const parts =
    Object
    .keys(globals)
        ...
        function preprocess(text, filename)
        {
            ...
            if (eslintEnvInfos.length)
            {
                ...
                for (const eslintEnvInfo of eslintEnvInfos)
                {
                    ...
                    const replacement = createReplacement(globals, eslintEnvInfo.lineBreak);
