...
const { commentParser, validatePlugins } =
(() =>
{
    ...
    return { commentParser, validatePlugins };
}
)();
...
const DISABLED_RULE_STATES =
['intersection', 'overlap', 'overlap-multiline', 'anywhere', 'anywhere-multiline'];
...
function getIndexFromMessageLoc(line, column, sourceCode)
{
    ...
    const index = sourceCode.getIndexFromLoc(loc) + sourceCode.hasBOM;
    return index;
}
...
(messages, disabledRules, originalSourceCode, processedSourceCode, eslintEnvInfos)
{
    ...
    function messageOverlapsWithMultilineEslintEnvComment({ line, column, endLine, endColumn })
    {
        const startIndex = getIndexFromMessageLoc(line, column, processedSourceCode);
        const endIndex = getIndexFromMessageLoc(endLine, endColumn, processedSourceCode);
        for (const eslintEnvInfo of eslintEnvInfos)
        {
            if (!eslintEnvInfo.multiline)
                continue;
            const { range } = eslintEnvInfo;
            if (range[1] > startIndex && range[0] < endIndex)
                return true;
        }
        return false;
    }
    ...
    function adjustLocation(message, lineKey, colKey)
    {
        ...
        if (originalIndex == null)
            return false;
        const originalLoc =
        originalSourceCode.getLocFromIndex(originalIndex - originalSourceCode.hasBOM);
    ...
    for (const message of messages)
    {
        ...
        case 'overlap':
            if (messageOverlapsWithEslintEnvComment(message))
                continue;
            break;
        case 'overlap-multiline':
            if (messageOverlapsWithMultilineEslintEnvComment(message))
                continue;
            break;
        case 'anywhere':
            continue;
        case 'anywhere-multiline':
            if (eslintEnvInfos.some(({ multiline }) => multiline))
                continue;
            break;
        }
        if
