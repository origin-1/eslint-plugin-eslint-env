...
function normalizeDisabledRules(rawDisabledRules = { })
{
    ...
    for (const [ruleId, value] of Object.entries(rawDisabledRules))
    {
        ...
        {
            const message =
            'Valid settings for disabledRules values are \'intersection\', \'overlap\', ' +
            `'always' or undefined, but ${inspect(value)} was specified for rule '${ruleId}'`;
            throw TypeError(message);
        }
    }
...
(messages, disabledRules, originalSourceCode, processedSourceCode, eslintEnvInfos)
{
    ...
    for (const message of messages)
    {
        ...
        if (disabledRuleState === 'anywhere')
            continue;
        if (disabledRuleState === 'overlap' && messageOverlapsWithEslintEnvComment(message))
            continue;
        if
        (
